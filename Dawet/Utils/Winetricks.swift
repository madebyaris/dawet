...
class Winetricks {
    static let winetricksURL: URL = DawetWineInstaller.libraryFolder
    ...
    static func runCommand(command: String, bottle: Bottle) async {
        ...
        let winetricksCmd = #"PATH=\"\#(DawetWineInstaller.binFolder.path):\#(resourcesURL.path(percentEncoded: false)):$PATH\" WINE=wine64 WINEPREFIX=\"\#(bottle.url.path)\" \"\#(winetricksURL.path(percentEncoded: false))\" \#(command)"#
    ...
    static func parseVerbs() async -> [WinetricksCategory] {
        // Grab the verbs file
        let verbsURL = DawetWineInstaller.libraryFolder.appending(path: "verbs.txt")
