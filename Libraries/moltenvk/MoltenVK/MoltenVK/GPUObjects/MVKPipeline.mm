...
bool MVKGraphicsPipeline::addVertexShaderToPipeline(MTLRenderPipelineDescriptor* plDesc,
	...
	// texture_offset_buffer_index was removed in newer SPIRV-Cross versions
	// Using dynamic_offsets_buffer_index instead if needed
...
bool MVKGraphicsPipeline::addVertexShaderToPipeline(MTLMeshRenderPipelineDescriptor* plDesc,
    ...
    shaderConfig.options.mslOptions.capture_output_to_buffer = false;
    shaderConfig.options.mslOptions.disable_rasterization = !_isRasterizing;
    // for_mesh_pipeline was removed in newer SPIRV-Cross versions
	...
	// input_primitive_type and PrimitiveTopology were removed in newer SPIRV-Cross versions
	// These options are no longer needed as SPIRV-Cross handles primitive types automatically
...
void MVKGraphicsPipeline::initShaderConversionConfig(SPIRVToMSLConversionConfiguration& shaderConfig,
	...
	// bitwise_not_causes_ice and add_texture_buffer_offsets were removed in newer SPIRV-Cross versions
...
static MVK_spirv_cross::MSLShaderVariableFormat toMslShaderFormat(MVKFormatType type) {
    switch (type) {
        // Enum values were simplified in newer SPIRV-Cross versions
        case kMVKFormatColorInt8: return MSL_SHADER_VARIABLE_FORMAT_UINT8; // INT8 -> UINT8
        case kMVKFormatColorUInt8: return MSL_SHADER_VARIABLE_FORMAT_UINT8;
        case kMVKFormatColorInt16: return MSL_SHADER_VARIABLE_FORMAT_ANY16; // INT16 -> ANY16
        case kMVKFormatColorUInt16: return MSL_SHADER_VARIABLE_FORMAT_UINT16;
        case kMVKFormatColorInt32: return MSL_SHADER_VARIABLE_FORMAT_ANY32; // INT32 -> ANY32
        case kMVKFormatColorUInt32: return MSL_SHADER_VARIABLE_FORMAT_ANY32; // UINT32 -> ANY32
        case kMVKFormatColorFloat: return MSL_SHADER_VARIABLE_FORMAT_ANY32; // FLOAT -> ANY32
        case kMVKFormatColorHalf: return MSL_SHADER_VARIABLE_FORMAT_ANY16; // HALF -> ANY16
...
void MVKGraphicsPipeline::addVertexInputToShaderConversionConfig(SPIRVToMSLConversionConfiguration& shaderConfig,
    ...
    for (uint32_t vaIdx = 0; vaIdx < vaCnt; vaIdx++) {
        ...
        // offset, stride, normalized, and binding members were removed from MSLShaderInterfaceVariable in newer SPIRV-Cross
        // These are now handled automatically by SPIRV-Cross
        // si.shaderVar.offset = pVKVA->offset;
        // si.shaderVar.stride = pVKVB->stride;

        // for_mesh_pipeline was removed in newer SPIRV-Cross versions
        // Check if this is a mesh pipeline using alternative method if needed
        if (false) { // shaderConfig.options.mslOptions.for_mesh_pipeline) {
