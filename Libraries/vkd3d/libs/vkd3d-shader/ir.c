...
struct io_normaliser
{
    ...
    struct io_normaliser_register_data input_range_map[MAX_REG_OUTPUT][VKD3D_VEC4_SIZE];
    struct io_normaliser_register_data output_range_map[MAX_REG_OUTPUT][VKD3D_VEC4_SIZE];
    struct io_normaliser_register_data pc_range_map[MAX_REG_OUTPUT][VKD3D_VEC4_SIZE];
...
static unsigned int range_map_get_register_count(struct io_normaliser_register_data range_map[][VKD3D_VEC4_SIZE],
        unsigned int register_idx, uint32_t write_mask)
{
    return range_map[register_idx][vsir_write_mask_get_component_idx(write_mask)].register_count;
}

static enum vkd3d_result range_map_set_register_range(struct io_normaliser *normaliser,
        struct io_normaliser_register_data range_map[][VKD3D_VEC4_SIZE], unsigned int register_idx,
        unsigned int register_count, unsigned int mask, unsigned int used_mask, bool is_dcl_indexrange)
{
    ...
    if (range_map[register_idx][component_idx].register_count > register_count && is_dcl_indexrange)
    {
        if (range_map[register_idx][component_idx].register_count == UINT8_MAX)
        {
            WARN("Conflicting index ranges.\n");
            vkd3d_shader_error(normaliser->message_context, &no_loc,
                    VKD3D_SHADER_ERROR_VSIR_INVALID_SIGNATURE, "Conflicting index ranges.");
            return VKD3D_ERROR_INVALID_SHADER;
        }
        return VKD3D_OK;
    }
    if (range_map[register_idx][component_idx].register_count == register_count)
    {
    ...
    range_map[register_idx][component_idx].register_count = register_count;
    range_map[register_idx][component_idx].mask = mask;
    range_map[register_idx][component_idx].used_mask = used_mask;
    ...
    for (i = 0; i < register_count; ++i)
    {
        ...
        for (j = !i; j < component_count; ++j)
        {
            ...
            if (range_map[r][c].register_count && is_dcl_indexrange)
            {
                WARN("Conflicting index ranges.\n");
                vkd3d_shader_error(normaliser->message_context, &no_loc,
                        VKD3D_SHADER_ERROR_VSIR_INVALID_SIGNATURE, "Conflicting index ranges.");
            ...
            range_map[r][c].register_count = UINT8_MAX;
            range_map[r][c].mask = mask;
            range_map[r][c].used_mask = used_mask;
        }
    }
        ...
        const struct vkd3d_shader_instruction *ins)
{
    ...
    struct io_normaliser_register_data (*range_map)[VKD3D_VEC4_SIZE];
    unsigned int reg_idx, mask, used_mask, i;
    ...
    for (i = 0; i < range->register_count; ++i)
    {
        ...
        element = vsir_signature_find_element_for_reg(signature, reg_idx + i, mask);
        if (element)
        {
            mask |= element->mask;
...
static enum vkd3d_result shader_signature_map_patch_constant_index_ranges(struct io_normaliser *normaliser,
        struct shader_signature *s, struct io_normaliser_register_data range_map[][VKD3D_VEC4_SIZE])
{
        ...
        bool is_patch_constant)
{
    ...
    for (i = 0, new_count = 0; i < element_count; ++i)
    {
        ...
        if (register_count > 0)
        {
            ...
            e->mask = range_map[e->register_index][vsir_write_mask_get_component_idx(e->mask)].mask;
            e->used_mask = range_map[e->register_index][vsir_write_mask_get_component_idx(e->mask)].used_mask;
        }
        ...
        struct vsir_transformation_context *ctx)
{
    ...
    if ((ret = shader_signature_merge(&normaliser, &program->input_signature, normaliser.input_range_map, false)) < 0
            || (ret = shader_signature_merge(&normaliser, &program->output_signature, normaliser.output_range_map, false)) < 0
            || (ret = shader_signature_merge(&normaliser, &program->patch_constant_signature, normaliser.pc_range_map, true)) < 0)
    {
        program->instructions = normaliser.instructions;
        return ret;
    }
    ...
    for (i = 0; i < instructions->count; ++i)
    {
        ...
        switch (ins->opcode)
        {
            ...
            case VKD3DSIH_GATHER4_PO_C:
                extended_offset = ins->opcode == VKD3DSIH_GATHER4_PO
                        || ins->opcode == VKD3DSIH_GATHER4_PO_C;
