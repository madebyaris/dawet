        ...
        struct vkd3d_shader_message_context *message_context, uint64_t config_flags)
{
    ...
    if (scan_descriptor_info->descriptor_count)
    {
        size_t byte_count = scan_descriptor_info->descriptor_count * sizeof(*scan_descriptor_info->descriptors);
        if (!(compiler->scan_descriptor_info.descriptors = vkd3d_malloc(byte_count)))
        {
            spirv_compiler_destroy(compiler);
            return NULL;
        }
        memcpy(compiler->scan_descriptor_info.descriptors, scan_descriptor_info->descriptors, byte_count);
        compiler->scan_descriptor_info.descriptor_count = scan_descriptor_info->descriptor_count;
    }
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    {
        bool is_uint64 = src[0].reg.data_type == VKD3D_DATA_UINT64;
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    if (dst_sin->reg.type != VKD3DSPR_NULL)
    {
        ...
        if (instruction->flags & VKD3DSI_PRECISE_XYZW)
            vkd3d_spirv_build_op_decorate(builder, sin_id, SpvDecorationNoContraction, NULL, 0);
    }
    ...
    if (dst_cos->reg.type != VKD3DSPR_NULL)
    {
        ...
        if (instruction->flags & VKD3DSI_PRECISE_XYZW)
            vkd3d_spirv_build_op_decorate(builder, cos_id, SpvDecorationNoContraction, NULL, 0);
    }
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    struct vkd3d_spirv_builder *builder = &compiler->spirv_builder;
    const struct vkd3d_shader_src_param *src = instruction->src;
    bool is_64_bit = src[0].reg.data_type == VKD3D_DATA_UINT64;
        ...
        const struct vkd3d_shader_compile_info *compile_info, struct vkd3d_shader_code *spirv)
{
    ...
    if (program->shader_version.major >= 6
            && !spirv_compiler_is_target_extension_supported(compiler,
                    VKD3D_SHADER_SPIRV_EXTENSION_EXT_DESCRIPTOR_INDEXING)
            && match_program_name("Diablo IV.exe")
            && (result = vkd3d_shader_normalise_descriptor_arrays(&program->instructions,
                    compile_info, &compiler->scan_descriptor_info, compiler->message_context)) < 0)
        return result;
    ...
    for (i = 0; i < instructions.count && result >= 0; ++i)
    {
        ...
        if (result < 0 || (i < instructions.count - 1 && instructions.elements[i + 1].opcode == VKD3DSIH_PHI))
            continue;
