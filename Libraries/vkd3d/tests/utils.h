...
static inline HRESULT dxc_compile(void *dxc_compiler, const WCHAR *profile, unsigned int compile_options,
        const WCHAR *entry_point, const char *hlsl, struct vkd3d_shader_code *blob_out)
{
...
{
    D3D12_SHADER_TYPE_CS,
    D3D12_SHADER_TYPE_PS,
    D3D12_SHADER_TYPE_VS,
    D3D12_SHADER_TYPE_HS,
    D3D12_SHADER_TYPE_DS,
    D3D12_SHADER_TYPE_GS,
};

static inline HRESULT dxc_compiler_compile_shader(void *dxc_compiler, enum d3d12_shader_type type,
        unsigned int compile_options, bool alternate_ep, const char *hlsl, ID3D10Blob **blob_out)
{
    ...
    {
        [D3D12_SHADER_TYPE_CS] = L"cs_6_0",
        [D3D12_SHADER_TYPE_PS] = L"ps_6_0",
        [D3D12_SHADER_TYPE_VS] = L"vs_6_0",
        [D3D12_SHADER_TYPE_HS] = L"hs_6_0",
        [D3D12_SHADER_TYPE_DS] = L"ds_6_0",
        [D3D12_SHADER_TYPE_GS] = L"gs_6_0",
    };
    ...
    *blob_out = NULL;
    ...
    if (alternate_ep)
    {
        if (type == D3D12_SHADER_TYPE_HS)
            entry_point = L"/Ehs_main";
        else if (type == D3D12_SHADER_TYPE_DS)
            entry_point = L"/Eds_main";
    }

    if (FAILED(hr = dxc_compile(dxc_compiler, shader_profiles[type], compile_options, entry_point, hlsl, &blob)))
        return hr;
